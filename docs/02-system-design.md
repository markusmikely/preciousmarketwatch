...
## 9. Deployment Topology (Railway Production)

### 9a. Service Responsibilities

Inside Railway, the system uses these services:
...
### 9b. Internal Networking Rules

In production (Railway), all services communicate via Railway internal DNS hostnames. **No service binds to localhost for cross-service communication.**

- **Postgres:** `postgres://USER:PASSWORD@PMW-Postgres:5432/DB_NAME`
- **Redis:** `redis://:PASSWORD@PMW-Redis:6379`

### 9c. Environment Strategy (Railway-Specific)

Each service has distinct env vars:

**PMW-Bridge**

- `DATABASE_URL` — Postgres connection
- `REDIS_URL` — Redis connection
- `ANTHROPIC_API_KEY` — LLM API (optional; Bridge does not execute agents)
...
| Component | Accepts triggers | Writes run record | Pushes Redis signal | Executes agent logic | Streams WebSocket |
|-----------|------------------|-------------------|---------------------|----------------------|-------------------|
| Bridge | ✅ | ✅ | ✅ | ❌ | ✅ |
| Agents | ❌ | ❌ (reads) | ✅ (signals) | ✅ | ❌ |

**Never let Bridge execute agent logic. Never let Agents serve HTTP.**
