#!/usr/bin/env npx tsx
/**
 * Load Historical Platinum Price Data (One-Time Migration)
 *
 * Fetches platinum data from configured API, filters to 1990+, inserts into metal_prices (WordPress DB).
 * Idempotent: uses INSERT IGNORE.
 *
 * Run: npm run load-platinum
 * Requires: PLATINUM_API_URL (returns [{date, price, source?}]) and DATABASE_URL or DB_*
 *
 * Note: Set PLATINUM_API_URL to an API that returns the same format as FreeGoldAPI.
 */

const API_URL = process.env.PLATINUM_API_URL;
...
async function fetchData(): Promise<ApiRecord[]> {
  if (!API_URL) {
    console.log(`[${METAL}] PLATINUM_API_URL not set — skipping`);
    process.exit(0);
  }
...
async function run() {
  ...
  try {
    const { inserted, skipped } = await insertMetalRecords(conn, METAL, records, "platinum_api");
