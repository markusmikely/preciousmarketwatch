"""Stage 4 — TopFactors (model-config patched)"""
...
class TopFactors(JSONOutputMixin, BaseAgent):
    ...
    def __init__(self):
        super().__init__(
            ...
            failure_config=FailureConfig(failure_message="Top factors analysis failed."),
        )
    ...
    async def run(self, state: dict) -> dict:
        ...
        try:
            ...
            return {
                "top_factors": output, "current_stage": "stage4.top_factors",
            ...
            self.log.error(f"TopFactors failed: {exc}")
            await self._write_stage_record(run_id, status="failed", attempt=1, error=str(exc))
            return {"top_factors": None, "current_stage": "stage4.top_factors", "status": "failed",
                    "errors": state.get("errors", []) + [{"stage": "stage4.top_factors", "error": str(exc)}]}

    def validate_output(self, raw_output: str) -> dict:
        data = super().validate_output(raw_output)
        factors = data.get("factors", [])
        if len(factors) != 5:
            raise ValueError(f"Expected 5 factors, got {len(factors)}")
        return data