...
class BriefBuilder(JSONOutputMixin, BaseAgent):
    ...
    async def run(self, state: dict) -> dict:
        ...
        for topic in all_topics:
            ...
            try:
                ...
                if result["status"] == "passed":
                    ...
                    self.log.info(f"✓ '{title}' → passed (affiliate={result['affiliate_name']}, "
        ...
        return {
            ...
            # Don't set status="failed" even if zero briefs passed.
            # Zero briefs is valid — review items are saved to DB.
    ...
    async def _process_one_topic(
        ...
        try:
            ...
            coherence = enrichments.get("coherence_score", 0)
        except Exception as exc:
            # LLM failed — release lock, add to review
        ...
        # 5. Brief passed — assemble and return
        brief = {
            "topic": topic,
    ...
    async def _save_to_db(self, run_id: int, result: dict) -> None:
        try:
            ...
            await infra.postgres.execute(
                ...
                result.get("fit_score"), result.get("coherence"),
                result["status"],
                result.get("reason"),
                json.dumps(result.get("brief")) if result.get("brief") else None,
                json.dumps(result.get("review_item", {})),
            )
        except Exception as exc:
